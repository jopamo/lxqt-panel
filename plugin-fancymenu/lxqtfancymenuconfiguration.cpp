/* plugin-fancymenu/lxqtfancymenuconfiguration.cpp
...
LXQtFancyMenuConfiguration::LXQtFancyMenuConfiguration(PluginSettings* settings,
  ...
  connect(ui->shortcutEd->addMenuAction(tr("Reset")), &QAction::triggered, this,
          &LXQtFancyMenuConfiguration::shortcutReset);
  ...
  connect(ui->autoSelCB, &QAbstractButton::toggled, this,
          [this](bool checked) { this->settings().setValue(QStringLiteral("autoSel"), checked); });
  connect(ui->autoSelSB, &QSpinBox::valueChanged, this,
          [this](int value) { this->settings().setValue(QStringLiteral("autoSelDelay"), value); });
...
void LXQtFancyMenuConfiguration::loadSettings() {
  ...
  ui->iconLE->setText(
      settings().value(QStringLiteral("icon"), QLatin1String(LXQT_GRAPHICS_DIR "/helix.svg")).toString());
  ...
  int buttRowPosIdx =
      ui->buttRowPosCB->findData(buttonsAtTop ? LXQtFancyMenuButtonPosition::Top : LXQtFancyMenuButtonPosition::Bottom);
  ...
  int categoryPosIdx = ui->categoryViewPosCB->findData(categoriesAtRight ? LXQtFancyMenuCategoryPosition::Right
...
void LXQtFancyMenuConfiguration::chooseIcon() {
  ...
  QDir dir = f.dir();
  QFileDialog* d =
      new QFileDialog(this, tr("Choose icon file"),
                      !f.filePath().isEmpty() && dir.exists() ? dir.path() : QLatin1String(LXQT_GRAPHICS_DIR),
                      tr("Images (*.svg *.png)"));
  d->setWindowModality(Qt::WindowModal);
  d->setAttribute(Qt::WA_DeleteOnClose);
  connect(d, &QFileDialog::fileSelected, this, [&](const QString& icon) { ui->iconLE->setText(icon); });
  d->show();
}

void LXQtFancyMenuConfiguration::chooseMenuFile() {
  QFileDialog* d =
      new QFileDialog(this, tr("Choose menu file"), QLatin1String("/etc/xdg/menus"), tr("Menu files (*.menu)"));
  d->setWindowModality(Qt::WindowModal);
  d->setAttribute(Qt::WA_DeleteOnClose);
  connect(d, &QFileDialog::fileSelected, this, [&](const QString& file) { ui->menuFilePathLE->setText(file); });
  d->show();
}
