 ...
 protected:
  void paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const override {
    if (isSeparator(index)) {
      ...
      painter->drawLine(rect.topLeft().x() + margin, rect.topLeft().y(), rect.topRight().x() - margin,
                        rect.topRight().y());
      painter->setPen(QPen(Qt::white));
      painter->drawLine(rect.topLeft().x() + margin, rect.topLeft().y() + 1, rect.topRight().x() - margin,
                        rect.topRight().y() + 1);
      painter->restore();
    }
    else {
...
LXQtFancyMenuWindow::LXQtFancyMenuWindow(QWidget* parent) : QWidget{parent, Qt::Popup} {
  ...
  connect(mCategoryView->selectionModel(), &QItemSelectionModel::currentChanged, this,
          &LXQtFancyMenuWindow::activateCategory);
...
void LXQtFancyMenuWindow::onAppViewCustomMenu(const QPoint& p) {
  ...
  connect(a, &QAction::triggered, [file] {
    ...
    if (QFile::exists(desktopFile)) {
      QMessageBox::StandardButton btn = QMessageBox::question(
          nullptr, tr("Question"), tr("A file with the same name already exists.\nDo you want to overwrite it?"));
  ...
  if (mAppMap->isFavorite(canonicalFile)) {
    ...
    connect(a, &QAction::triggered, this, [this, canonicalFile] { removeFromFavorites(canonicalFile); });
  }
  else {
    a = menu.addAction(XdgIcon::fromTheme(QLatin1String("bookmark-new")), tr("Add to Favorites"));
    connect(a, &QAction::triggered, this, [this, canonicalFile] { addToFavorites(canonicalFile); });
  }
...
bool LXQtFancyMenuWindow::eventFilter(QObject* watched, QEvent* e) {
  if (e->type() == QEvent::KeyPress &&
      (watched == mSearchEdit || watched == mCategoryView->viewport() || watched == mAppView->viewport())) {
    ...
    else if (ev->key() == Qt::Key_Up || ev->key() == Qt::Key_PageUp || ev->key() == Qt::Key_Down ||
  ...
  else if (e->type() == QEvent::MouseButtonRelease && (watched == mAppView->viewport())) {
  ...
  else if (mAutoSel && (watched == mCategoryView->viewport() || watched == mAppView->viewport())) {
...
void LXQtFancyMenuWindow::showEvent(QShowEvent* e) {
  ...
  if (difW > 0 || difH > 0) {
    ...
    if (difW > 0) {  // also take stretch factors into account
      newWidth += qreal((CAT_VIEW_STRETCH + APP_VIEW_STRETCH) * difW) / CAT_VIEW_STRETCH + 1 +
                  2 * mCategoryView->lineWidth() + style()->pixelMetric(QStyle::PM_ScrollBarExtent);
    }
    if (difH > 0) {
      newHeight += difH + 2 * mCategoryView->lineWidth() + style()->pixelMetric(QStyle::PM_ScrollBarExtent);
    }
...
void LXQtFancyMenuWindow::runCommandHelper(const QString& cmd) {
  if (QProcess::startDetached(cmd, QStringList())) {
    ...
    QMessageBox::warning(this, tr("No Executable"), tr("Cannot find <b>%1</b> executable.").arg(cmd));
  }
}
...
void LXQtFancyMenuWindow::removeFromFavorites(const QString& desktopFile) {
  ...
  if (mAppMap->getFavoriteCount() == 0) {
    ...
    if (idx.row() == LXQtFancyMenuAppMap::FavoritesCategory && mCategoryView->selectionModel()->isSelected(idx)) {
