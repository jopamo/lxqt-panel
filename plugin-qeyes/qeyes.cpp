/* plugin-qeyes/qeyes.cpp
 * Implementation file for qeyes
 */
...
QEyesPlugin::QEyesPlugin(const ILXQtPanelPluginStartupInfo& startupInfo) : QObject(), ILXQtPanelPlugin(startupInfo) {
  w0 = new QWidget();
  l = new QVBoxLayout();
  l->setSpacing(0);
...
void QEyesPlugin::settingsChanged() {
  ...
  const auto type = _settings->value(QStringLiteral("eye_type"), internalEye).toString();
  ...
  if (type != internalEye && !vectorEyes) {
    if (!loadImage(type, dynamic_cast<QEyesImageWidget*>(w))) {
      ...
      _settings->setValue(QStringLiteral("eye_type"), internalEye);
      std::cerr << "ERROR: crash during load image\n";
    }
  }

  w->setNumEyes(_settings->value(QStringLiteral("num_eyes"), QLatin1String("2")).toInt());
  realign();
}
