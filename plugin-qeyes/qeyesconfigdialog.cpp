...
QEyesConfigDialog::QEyesConfigDialog(PluginSettings* sts, ILXQtPanelPlugin* plugin_, QWidget* parent)
  ...
  connect(numEyesWidget, qOverload<int>(&QSpinBox::valueChanged), this, &QEyesConfigDialog::updateValues);
  connect(typesWidget, qOverload<int>(&QComboBox::currentIndexChanged), this, &QEyesConfigDialog::updateValues);
}

void QEyesConfigDialog::showEvent(QShowEvent*) {
  old_num_eyes = _settings->value(QStringLiteral("num_eyes"), QStringLiteral("2")).toInt();
  old_type_eyes = _settings->value(QStringLiteral("eye_type"), QEyesPlugin::internalEye).toString();
...
void QEyesConfigDialog::updateValues(int) {
  _settings->setValue(QStringLiteral("num_eyes"), numEyesWidget->value());

  if (typesWidget->currentIndex() == 0 || !types.contains(typesWidget->currentText())) {
    _settings->setValue(QStringLiteral("eye_type"), QEyesPlugin::internalEye);
  }
  else {
    _settings->setValue(QStringLiteral("eye_type"), types[typesWidget->currentText()]);
  }
  _settings->sync();
...
void QEyesConfigDialog::buildList() {
  types.clear();
  for (const char* root : {LXQT_SHARE_DIR "/panel/qeyes-types/", "$HOMEDATA"}) {
    ...
    for (auto const& path : list) {
      const auto fn = QString(QLatin1String(root) + QStringLiteral("/") + path + QStringLiteral("/config"));
      QFileInfo f(fn);
      ...
      types[path] = QString(QLatin1String(root) + QStringLiteral("/") + path);
    }
  }
}