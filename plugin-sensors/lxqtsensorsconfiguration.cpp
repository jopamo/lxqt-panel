/* plugin-sensors/lxqtsensorsconfiguration.cpp
...
LXQtSensorsConfiguration::LXQtSensorsConfiguration(PluginSettings* settings, QWidget* parent)
    : LXQtPanelPluginConfigDialog(settings, parent), ui(new Ui::LXQtSensorsConfiguration), mLockSettingChanges(false) {
...
void LXQtSensorsConfiguration::saveSettings() {
  ...
  if (chipNames.size()) {
    ...
    for (int j = 0; j < chipFeatureLabels.size(); ++j) {
      ...
      settings().setValue(QStringLiteral("color"),
  ...
  settings().setValue(QStringLiteral("warningAboutHighTemperature"), ui->warningAboutHighTemperatureChB->isChecked());
}
...
void LXQtSensorsConfiguration::detectedChipSelected(int index) {
  ...
  if (index < chipNames.size()) {
    ...
    for (int j = 0; j < chipFeatureLabels.size(); ++j) {
      ...
      pal.setColor(QPalette::Normal, QPalette::Button, QColor(settings().value(QStringLiteral("color")).toString()));
