/* plugin-sensors/lxqtsensors.cpp
...
LXQtSensors::LXQtSensors(ILXQtPanelPlugin* plugin, QWidget* parent)
    : QFrame(parent), mPlugin(plugin), mSettings(plugin->settings()) {
...
void LXQtSensors::updateSensorReadings() {
  ...
  QList<ProgressBar*>::iterator temperatureProgressBarsIt = mTemperatureProgressBars.begin();
...
void LXQtSensors::warningAboutHighTemperature() {
  // Iterator for temperature progress bars
  QSet<ProgressBar*>::iterator temperatureProgressBarsIt = mHighTemperatureProgressBars.begin();

  int curValue;
  int maxValue;

  for (; temperatureProgressBarsIt != mHighTemperatureProgressBars.end(); ++temperatureProgressBarsIt) {
...
void ProgressBar::setSensorColor(const QString& colorName) {
  ...
  static const QString ss = QStringLiteral(
      "QProgressBar{background-color: %1; border-radius: 2px; border: 1px solid %2;}"
  ...
  outline.setRgbF(0.5 * base.redF() + 0.5 * outline.redF(), 0.5 * base.greenF() + 0.5 * outline.greenF(),
