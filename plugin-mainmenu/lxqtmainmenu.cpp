...
LXQtMainMenu::LXQtMainMenu(const ILXQtPanelPluginStartupInfo& startupInfo)
  ...
  connect(mSearchView, &QWidget::customContextMenuRequested, this,
          std::bind(&LXQtMainMenu::onRequestingCustomMenu, this, std::placeholders::_1, mSearchView));
  mSearchViewAction->setDefaultWidget(mSearchView);
  mSearchEdit = new QLineEdit;
  mSearchEdit->setClearButtonEnabled(true);
  mSearchEdit->setPlaceholderText(LXQtMainMenu::tr("Search..."));
  connect(mSearchEdit, &QLineEdit::textChanged, this, [this](QString const&) { mSearchTimer.start(); });
...
static bool filterMenu(QMenu* menu, QString const& filter) {
  ...
  for (auto const& action : actions) {
    if (QMenu* sub_menu = action->menu()) {
      ...
      bool visible(filter.isEmpty() || action->text().contains(filter, Qt::CaseInsensitive) ||
                   action->toolTip().contains(filter, Qt::CaseInsensitive));
...
void LXQtMainMenu::addContextMenu(QMenu* menu) {
  ...
  for (auto const& action : actions) {
    if (action->menu()) {
      ...
      connect(action->menu(), &QWidget::customContextMenuRequested, this,
              std::bind(&LXQtMainMenu::onRequestingCustomMenu, this, std::placeholders::_1, action->menu()));
...
void LXQtMainMenu::onRequestingCustomMenu(const QPoint& p, QObject* sender) {
  ...
  connect(a, &QAction::triggered, this, [file] {
    ...
    data->setData(QStringLiteral("text/uri-list"), QUrl::fromLocalFile(file).toEncoded() + QByteArray("\r\n"));
...
void LXQtMainMenu::setMenuFontSize() {
  ...
  int icon_size = (customFont ? QFontMetrics(menuFont).height() : MenuStyle::DEFAULT_ICON_SIZE);
...
void LXQtMainMenu::setButtonIcon() {
  if (settings()->value(QStringLiteral("ownIcon"), false).toBool()) {
    mButton.setStyleSheet(
        QStringLiteral("#MainMenu { qproperty-icon: url(%1); }")
...
bool LXQtMainMenu::eventFilter(QObject* obj, QEvent* event) {
  if (obj == mButton.parentWidget()) {
    ...
    if (obj == mMenu) {
      ...
      else if (QEvent::ActionChanged == event->type() || QEvent::ActionAdded == event->type()) {
  ...
  if (event->type() == QEvent::MouseButtonRelease) {
    ...
    QObject* sender =
        (obj == mSearchView->viewport() ? static_cast<QObject*>(mSearchView) : (menu != nullptr ? menu : nullptr));
