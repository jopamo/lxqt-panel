/* plugin-mainmenu/actionview.cpp
...
class DelayedIconDelegate : public QStyledItemDelegate {
 public:
  DelayedIconDelegate(QObject* parent = nullptr) : QStyledItemDelegate(parent) {}

  void setMaxItemWidth(int max) { mMaxItemWidth = max; }
...
ActionView::ActionView(QWidget* parent /*= nullptr*/)
    : QListView(parent), mModel{new QStandardItemModel{this}}, mProxy{new FilterProxyModel{this}}, mMaxItemsToShow(10) {
...
void ActionView::fillActionsRecursive(QMenu* menu) {
  ...
  for (auto const& action : actions) {
    ...
    else if (nullptr == qobject_cast<QWidgetAction*>(action) && !action->isSeparator()) {
