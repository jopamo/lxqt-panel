 ...
 * Implementation file for lxqtcustomcommand
 */
...
void LXQtCustomCommand::settingsChanged() {
  ...
  if (settings()->contains(QStringLiteral("outputFormat")))
    mOutputFormat = static_cast<LXQtCustomCommandConfiguration::OutputFormat_t>(
        settings()->value(QStringLiteral("outputFormat")).toInt());
  else
    mOutputFormat = settings()->value(QStringLiteral("outputImage"), false).toBool() ? OutputFormat_t::OUTPUT_ICON
                                                                                     : OutputFormat_t::OUTPUT_TEXT;
  ...
  if (oldCommand != mCommand || oldRunWithBash != mRunWithBash || oldOutputFormat != mOutputFormat ||
      oldContinuousOutput != mContinuousOutput || oldRepeat != mRepeat)
    shouldRun = true;
...
void LXQtCustomCommand::handleFinished(int exitCode, QProcess::ExitStatus exitStatus) {
  if (!mTerminating) {
    if (exitStatus != QProcess::NormalExit || exitCode != 0)
      qWarning().nospace() << "customcommand: non-gracefull command finish(" << exitStatus << ',' << exitCode
    ...
    case OutputFormat_t::OUTPUT_STRUCTURED:
      if (mOutputByteArray.size() > 0) {
        for (const auto& variable : mOutputByteArray.split(' ')) {
          ...
          if (error) {
            qWarning().nospace() << "customcommand: Can't parse name-value("
                                 << (name_value.empty() ? QString{} : QString::fromUtf8(name_value[0]))
          ...
          if (name_value[0] == "text") {
            ...
            qWarning().nospace() << "customcommand: Unsupported parameter(" << QString::fromUtf8(name_value[0])
                                 << ") to set";
          }
        }
      }
