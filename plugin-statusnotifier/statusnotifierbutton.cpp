...
class MenuImporter : public QObject {
 public:
  MenuImporter(const QString&, const QString&, QObject* parent) : QObject(parent) {
    ...
    m_menu->addAction(QObject::tr("Menu not available (Stub)"));
  }
...
StatusNotifierButton::StatusNotifierButton(QString service,
  ...
  interface->propertyGetAsync(QLatin1String("Status"), [this](QString status) { newStatus(status); });
...
void StatusNotifierButton::newIcon() {
  ...
  interface->propertyGetAsync(QLatin1String("IconThemePath"), [this](QString value) { refetchIcon(Passive, value); });
}

void StatusNotifierButton::newOverlayIcon() {
  onNeedingAttention();

  interface->propertyGetAsync(QLatin1String("IconThemePath"), [this](QString value) { refetchIcon(Active, value); });
}
  ...
  interface->propertyGetAsync(nameProperty, [this, status, pixmapProperty, themePath](QString iconName) {
    if (!iconName.isEmpty()) {
      ...
      if (nextIcon.isNull()) {
        ...
        if (themeDir.exists()) {
          bool hasExtension = iconName.endsWith(QStringLiteral(".png")) || iconName.endsWith(QStringLiteral(".svg")) ||
                              iconName.endsWith(QStringLiteral(".xpm"));
          if (hasExtension) {  // extension is included
          ...
          if (themeDir.cd(QStringLiteral("hicolor")) ||
              (themeDir.cd(QStringLiteral("icons")) && themeDir.cd(QStringLiteral("hicolor")))) {
            ...
            for (const QString& dir : sizes) {
              ...
              for (const QString& innerDir : dirs) {
                QString path = themeDir.absolutePath() + QLatin1Char('/') + dir + QLatin1Char('/') + innerDir +
