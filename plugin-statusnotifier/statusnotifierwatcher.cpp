/* plugin-statusnotifier/statusnotifierwatcher.cpp
 * Implementation file for statusnotifierwatcher
 */
...
StatusNotifierWatcher::StatusNotifierWatcher(QObject* parent) : QObject(parent) {
  ...
  switch (dbus.interface()
              ->registerService(QStringLiteral("org.kde.StatusNotifierWatcher"), QDBusConnectionInterface::QueueService)
              .value()) {
    ...
    case QDBusConnectionInterface::ServiceQueued:
      qWarning() << "StatusNotifier: registration of service org.kde.StatusNotifierWatcher queued, we can become "
