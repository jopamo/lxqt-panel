...
template <typename>
struct remove_class_type {
  using type = void;
  using arg_type = void;
};  // bluff
template <typename C, typename R, typename... ArgTypes>
struct remove_class_type<R (C::*)(ArgTypes...)> {
  using type = R(ArgTypes...);
  using arg_type = std::tuple_element_t<0, std::tuple<ArgTypes...>>;
};
template <typename C, typename R, typename... ArgTypes>
struct remove_class_type<R (C::*)(ArgTypes...) const> {
  using type = R(ArgTypes...);
  using arg_type = std::tuple_element_t<0, std::tuple<ArgTypes...>>;
};
...
struct call_signature<R(ArgTypes...)> {
  using type = R(ArgTypes...);
  using arg_type = std::tuple_element_t<0, std::tuple<ArgTypes...>>;
};
template <typename R, typename... ArgTypes>
...
template <typename C, typename R, typename... ArgTypes>
struct call_signature<R (C::*)(ArgTypes...)> {
  using type = R(ArgTypes...);
  using arg_type = std::tuple_element_t<0, std::tuple<ArgTypes...>>;
};
template <typename C, typename R, typename... ArgTypes>
struct call_signature<R (C::*)(ArgTypes...) const> {
  using type = R(ArgTypes...);
  using arg_type = std::tuple_element_t<0, std::tuple<ArgTypes...>>;
};

template <typename>
struct is_valid_signature : public std::false_type {};
...
class SniAsync : public QObject {
 ...
 public:
  ...
  inline void propertyGetAsync(QString const& name, F finished) {
    ...
    connect(new QDBusPendingCallWatcher{asyncPropGet(name), this}, &QDBusPendingCallWatcher::finished, this,
            [this, finished, name](QDBusPendingCallWatcher* call) {
              QDBusPendingReply<QVariant> reply = *call;
              if (reply.isError() && ignored_errors.cend() == std::find(ignored_errors.cbegin(), ignored_errors.cend(),
                                                                        reply.error().name()))
