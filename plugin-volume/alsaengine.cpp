...
MixerHandler::MixerHandler(snd_mixer_t* mixer, QObject* parent /*= nullptr*/) : QObject{parent}, m_mixer{mixer} {
  if (nullptr != m_mixer) {
...
AlsaEngine::AlsaEngine(QObject* parent) : AudioEngine(parent) {
  discoverDevices();
  m_instance = this;
}
...
void AlsaEngine::commitDeviceVolume(AudioDevice* device) {
  ...
  long value =
      dev->volumeMin() + std::round(static_cast<double>(dev->volume()) / 100.0 * (dev->volumeMax() - dev->volumeMin()));
...
void AlsaEngine::updateDevice(AlsaDevice* device) {
  ...
  device->setVolumeNoCommit(std::round((static_cast<double>(value - device->volumeMin()) * 100.0) /
                                       (device->volumeMax() - device->volumeMin())));
