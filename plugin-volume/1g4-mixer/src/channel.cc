/* plugin-volume/1g4-mixer/src/channel.cc
 * Implementation of channel.cc
 */
...
Channel::Channel(QGridLayout* parent) : QObject(parent), can_decibel(false), volumeScaleEnabled(true), last(false) {
  ...
  volumeLabel->setFixedWidth(
      QFontMetrics{volumeLabel->font()}.size(Qt::TextSingleLine, QStringLiteral("100%(-99.99dB)")).width());
...
void Channel::setVolume(pa_volume_t volume) {
  ...
  if (can_decibel) {
    ...
    volumeLabel->setText(
        tr("%1% (%2dB)", "volume slider label [X% (YdB)]")
            .arg(v)
