/* plugin-volume/1g4-mixer/src/mainwindow.cc
 * Implementation of mainwindow.cc
 */
...
MainWindow::MainWindow()
  ...
  connect(sinkInputTypeComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &MainWindow::onSinkInputTypeComboBoxChanged);
  connect(sourceOutputTypeComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &MainWindow::onSourceOutputTypeComboBoxChanged);
  connect(sinkTypeComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &MainWindow::onSinkTypeComboBoxChanged);
  connect(sourceTypeComboBox, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &MainWindow::onSourceTypeComboBoxChanged);
...
static void updatePorts(DeviceWidget* w, std::map<QByteArray, PortInfo>& ports) {
  ...
  for (auto& port : w->ports) {
    ...
    if (p.available == PA_PORT_AVAILABLE_YES)
      ...
      if (p.name == "analog-output-speaker" || p.name == "analog-input-microphone-internal")
...
pa_stream* MainWindow::createMonitorStreamForSource(uint32_t source_idx,
                                                    uint32_t stream_idx = -1,
                                                    bool suspend = false) {
  pa_stream* s;
  char t[16];
  pa_buffer_attr attr;
...
void MainWindow::updateSource(const pa_source_info& info) {
  ...
  w->name = info.name;
  w->description = info.description;
  w->type = info.monitor_of_sink != PA_INVALID_INDEX
                ? SOURCE_MONITOR
...
void MainWindow::updateSinkInput(const pa_sink_input_info& info) {
  ...
  if ((t = pa_proplist_gets(info.proplist, "module-stream-restore.id"))) {
    if (strcmp(t, "sink-input-by-media-role:event") == 0) {
      g_debug("%s",
              tr("Ignoring sink-input due to it being designated as an event and thus handled by the Event widget")
                  .toUtf8()
                  .constData());
      return;
    }
...
void MainWindow::updateSourceOutput(const pa_source_output_info& info) {
  ...
  if ((app = pa_proplist_gets(info.proplist, PA_PROP_APPLICATION_ID)))
    if (strcmp(app, "org.PulseAudio.1g4-mixer") == 0 || strcmp(app, "org.gnome.VolumeControl") == 0 ||
...
bool MainWindow::createEventRoleWidget() {
  ...
  pa_channel_map cm = {1, {PA_CHANNEL_POSITION_MONO}};
