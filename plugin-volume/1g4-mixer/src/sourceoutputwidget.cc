/* plugin-volume/1g4-mixer/src/sourceoutputwidget.cc
 * Implementation of sourceoutputwidget.cc
 */
...
SourceOutputWidget::SourceOutputWidget(MainWindow* parent) : StreamWidget(parent), menu{new QMenu{this}} {
...
void SourceOutputWidget::buildMenu() {
  for (auto& sourceWidget : mpMainWindow->sourceWidgets) {
    menu->addAction(new SourceMenuItem{this, sourceWidget.second->description.constData(), sourceWidget.second->index,
                                       sourceWidget.second->index == mSourceIndex, menu});
  }
}
