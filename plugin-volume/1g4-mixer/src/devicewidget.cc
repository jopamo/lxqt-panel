/* plugin-volume/1g4-mixer/src/devicewidget.cc
 * Implementation of devicewidget.cc
 */
...
DeviceWidget::DeviceWidget(MainWindow* parent, QByteArray deviceType)
  ...
  connect(portList, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &DeviceWidget::onPortChange);
  connect(offsetButton, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this,
          &DeviceWidget::onOffsetChange);
...
void DeviceWidget::onOffsetChange() {
  ...
  if (!(o = pa_context_set_port_latency_offset(get_context(), card_name.constData(), activePort.constData(), offset,
                                               nullptr, nullptr))) {
...
void DeviceWidget::renamePopup() {
  ...
  if (!mpMainWindow->canRenameDevices) {
    QMessageBox::warning(
        this, tr("Sorry, but device renaming is not supported."),
  ...
  const QString new_name =
      QInputDialog::getText(this, QCoreApplication::organizationName(), tr("Rename device %1 to:").arg(old_name),
                            QLineEdit::Normal, old_name, &ok);
  if (ok && new_name != old_name) {
    pa_operation* o;
    gchar* key = g_markup_printf_escaped("%s:%s", mDeviceType.constData(), name.constData());

    if (!(o = pa_ext_device_manager_set_device_description(get_context(), key, new_name.toUtf8().constData(), nullptr,
                                                           nullptr)))
