 ...
 public:
  ...
  bool init() {
    ...
    xkb_x11_setup_xkb_extension(m_connection, XKB_X11_MIN_MAJOR_XKB_VERSION, XKB_X11_MIN_MINOR_XKB_VERSION,
                                XKB_X11_SETUP_XKB_EXTENSION_NO_FLAGS, nullptr, nullptr, &m_eventType, nullptr);
    ...
    if ((event->response_type & ~0x80) == m_eventType) {
      ...
      switch (sevent->xkbType) {
        case XCB_XKB_STATE_NOTIFY:
          xkb_state_update_mask(m_state, sevent->baseMods, sevent->latchedMods, sevent->lockedMods, sevent->baseGroup,
                                sevent->latchedGroup, sevent->lockedGroup);
  ...
  bool isModifierLocked(Controls cnt) const { return m_modifiers[cnt]; }
        ...
        if (file.open(QIODevice::ReadOnly)) {
          if (doc.setContent(&file)) {
            ...
            for (int i = 0; i < layout.childNodes().count(); ++i) {
              ...
              names.insert(conf.firstChildElement(QStringLiteral("description")).firstChild().toText().data(),
              ...
              for (int j = 0; j < variants.childNodes().count(); ++j) {
                ...
                names.insert(var.firstChildElement(QStringLiteral("description")).firstChild().toText().data(),
                             {conf.firstChildElement(QStringLiteral("description")).firstChild().toText().data(),
                              conf.firstChildElement(QStringLiteral("name")).firstChild().toText().data(),
...
X11Kbd::X11Kbd() : m_priv(new pimpl::X11Kbd(this)) {}
