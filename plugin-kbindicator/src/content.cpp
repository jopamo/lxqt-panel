/* plugin-kbindicator/src/content.cpp
...
Content::Content(bool layoutEnabled) : QWidget(), m_layoutEnabled(layoutEnabled) {
...
void Content::layoutChanged(const QString& sym, const QString& name, const QString& variant) {
  ...
  if (flag_file.isEmpty() || !QFileInfo::exists(flag_file)) {
    ...
    m_layout->setToolButtonStyle(m_layout->icon().pixmap(m_layout->iconSize()).isNull() ? Qt::ToolButtonTextOnly
