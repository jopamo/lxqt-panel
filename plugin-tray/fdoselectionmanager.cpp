/* plugin-tray/fdoselectionmanager.cpp
 * Implementation file for fdoselectionmanager
 */
...
FdoSelectionManager::FdoSelectionManager() : m_atoms{nullptr}, m_selectionOwner{nullptr} {
...
void FdoSelectionManager::init() {
  ...
  connect(m_selectionOwner, &KSelectionOwner::failedToClaimOwnership, this,
          &FdoSelectionManager::onFailedToClaimOwnership);
...
bool FdoSelectionManager::addDamageWatch(xcb_window_t client) {
  ...
  Xcb::ScopedCPointer<xcb_get_window_attributes_reply_t> attr(
      xcb_get_window_attributes_reply(m_connection, attribsCookie, &error));
  ...
  // the event mask will not be removed again. We cannot track whether another component also needs STRUCTURE_NOTIFY
  // (e.g. KWindowSystem). if we would remove the event mask again, other areas will break.
...
void FdoSelectionManager::setSystemTrayVisual() {
  ...
  xcb_change_property(m_connection, XCB_PROP_MODE_REPLACE, m_selectionOwner->ownerWindow(), m_atoms->visualAtom,
                      XCB_ATOM_VISUALID, 32, 1, &trayVisual);
}