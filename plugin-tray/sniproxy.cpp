 ...
 * Implementation file for sniproxy
 */
...
void xembed_message_send(xcb_connection_t* conn,
                         Xcb::Atoms& atoms,
                         xcb_window_t towin,
                         long message,
                         long d1,
...
SNIProxy::SNIProxy(xcb_window_t wid, Xcb::Atoms& atoms, QObject* parent)
    : QObject(parent),
      ...
      // there is an undocumented feature that you can register an SNI by path, however it doesn't detect an object on a
      // service being removed, only the entire service closing instead lets use one DBus connection per SNI
      m_dbus(QDBusConnection::connectToBus(QDBusConnection::SessionBus,
                                           QStringLiteral("XembedSniProxy%1").arg(s_serviceCount++))),
  ...
  auto statusNotifierWatcher = new org::kde::StatusNotifierWatcher(
      QStringLiteral(SNI_WATCHER_SERVICE_NAME), QStringLiteral(SNI_WATCHER_PATH), QDBusConnection::sessionBus(), this);
  ...
  values[2] = XCB_EVENT_MASK_VISIBILITY_CHANGE |  // receive visibility change, to handle KWin restart #357443
                                                  // Redirect and handle structure (size, position) requests from the
                                                  // embedded window.
  ...
  /*
      ...
      We also need it to exist in the right place to get the clicks working as GTK will check sendEvent locations to see
     if our window is in the right place. So even though our contents are drawn via compositing we still put this window
     in the right place

      We can't composite it away anything parented owned by the root window (apparently)
      Stack Under works in the non composited case, but it doesn't seem to work in kwin's composited case (probably need
     set relevant NETWM hint)
  ...
  */
  ...
  // note also some clients might not have the XTest extension. We may as well assume it does and just fail to send
  // later.
  ...
  Xcb::ScopedCPointer<xcb_get_window_attributes_reply_t> windowAttributes(
      xcb_get_window_attributes_reply(m_connection, waCookie, nullptr));
...
void SNIProxy::resizeWindow(const uint16_t width, const uint16_t height) const {
  ...
  xcb_configure_window(m_connection, m_windowId, XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT,
...
QSize SNIProxy::calculateClientWindowSize() const {
  ...
  // this is needed as chromium and such when resized just fill the icon with transparent space and only draw in the
  // middle however KeePass2 does need this as by default the window size is 273px wide and is not transparent use an
  // arbitrary heuristic to make sure icons are always sensible
...
QImage SNIProxy::getImageNonComposite() const {
  ...
  xcb_image_t* image = xcb_image_get(m_connection, m_windowId, 0, 0, clientWindowSize.width(),
                                     clientWindowSize.height(), 0xFFFFFFFF, XCB_IMAGE_FORMAT_Z_PIXMAP);
  ...
  if (isTransparentImage(naiveConversion)) {
    ...
    return QImage(image->data, image->width, image->height, image->stride, QImage::Format_ARGB32, sni_cleanup_xcb_image,
                  image);
  }
}
...
QImage SNIProxy::convertFromNative(xcb_image_t* xcbImage) const {
  ...
  QImage image(xcbImage->data, xcbImage->width, xcbImage->height, xcbImage->stride, format, sni_cleanup_xcb_image,
               xcbImage);
...
QPoint SNIProxy::calculateClickPoint() const {
  ...
  xcb_shape_get_rectangles_cookie_t rectaglesCookie =
      xcb_shape_get_rectangles(m_connection, m_windowId, XCB_SHAPE_SK_BOUNDING);

  Xcb::ScopedCPointer<xcb_shape_query_extents_reply_t> extentsReply(
      xcb_shape_query_extents_reply(m_connection, extentsCookie, nullptr));
  Xcb::ScopedCPointer<xcb_shape_get_rectangles_reply_t> rectanglesReply(
      xcb_shape_get_rectangles_reply(m_connection, rectaglesCookie, nullptr));
...
void SNIProxy::sendClick(uint8_t mouseButton, int x, int y) {
  ...
  // in order to solve this we move the embed container over to where the mouse is then replay the event using
  // send_event if patching, test with xchat + xchat context menus
