 ...
 * Implementation file for lxqtsysstat
 */
...
LXQtSysStatTitle::LXQtSysStatTitle(QWidget* parent) : QLabel(parent) {}
...
void LXQtSysStatContent::mixNetColours() {
  ...
  if (std::abs(netReceivedColour_hsv.hueF() - netTransmittedColour_hsv.hueF()) > 0.5)
    hue += 0.5;
  mNetBothColour.setHsvF(hue, (netReceivedColour_hsv.saturationF() + netTransmittedColour_hsv.saturationF()) / 2,
...
void LXQtSysStatContent::updateSettings(const PluginSettings* settings) {
  ...
  mNetMaximumSpeed = PluginSysStat::netSpeedFromString(
  ...
  mSettingsColours.gridColour =
      QColor(settings->value(QStringLiteral("grid/colour"), QStringLiteral("#c0c0c0")).toString());

  mSettingsColours.titleColour =
      QColor(settings->value(QStringLiteral("title/colour"), QStringLiteral("#ffffff")).toString());

  mSettingsColours.cpuSystemColour =
      QColor(settings->value(QStringLiteral("cpu/systemColour"), QStringLiteral("#800000")).toString());
  mSettingsColours.cpuUserColour =
      QColor(settings->value(QStringLiteral("cpu/userColour"), QStringLiteral("#000080")).toString());
  mSettingsColours.cpuNiceColour =
      QColor(settings->value(QStringLiteral("cpu/niceColour"), QStringLiteral("#008000")).toString());
  mSettingsColours.cpuOtherColour =
      QColor(settings->value(QStringLiteral("cpu/otherColour"), QStringLiteral("#808000")).toString());
  mSettingsColours.frequencyColour =
      QColor(settings->value(QStringLiteral("cpu/frequencyColour"), QStringLiteral("#808080")).toString());

  mSettingsColours.memAppsColour =
      QColor(settings->value(QStringLiteral("mem/appsColour"), QStringLiteral("#000080")).toString());
  mSettingsColours.memBuffersColour =
      QColor(settings->value(QStringLiteral("mem/buffersColour"), QStringLiteral("#008000")).toString());
  mSettingsColours.memCachedColour =
      QColor(settings->value(QStringLiteral("mem/cachedColour"), QStringLiteral("#808000")).toString());
  mSettingsColours.swapUsedColour =
      QColor(settings->value(QStringLiteral("mem/swapColour"), QStringLiteral("#800000")).toString());

  mSettingsColours.netReceivedColour =
      QColor(settings->value(QStringLiteral("net/receivedColour"), QStringLiteral("#000080")).toString());
  mSettingsColours.netTransmittedColour =
      QColor(settings->value(QStringLiteral("net/transmittedColour"), QStringLiteral("#808000")).toString());
    ...
    if (needReconnecting) {
      if (mDataType == QLatin1String("CPU")) {
        ...
        if (mUseFrequency) {
          ...
          connect(cpustat, QOverload<float, float, float, float, float, uint>::of(&SysStat::CpuStat::update), this,
                  &LXQtSysStatContent::cpuLoadFrequencyUpdate);
        }
        else {
          cpustat->setMonitoring(SysStat::CpuStat::LoadOnly);
          connect(cpustat, QOverload<float, float, float, float>::of(&SysStat::CpuStat::update), this,
                  &LXQtSysStatContent::cpuLoadUpdate);
        }
      }
      else if (mDataType == QLatin1String("Memory")) {
...
void LXQtSysStatContent::cpuLoadFrequencyUpdate(float user,
                                                float nice,
                                                float system,
                                                float other,
                                                float frequencyRate,
...
void LXQtSysStatContent::paintEvent(QPaintEvent* event) {
  ...
  p.drawImage(QRect(0, -height(), width() - mHistoryOffset, graphHeight), mHistoryImage,
              QRect(mHistoryOffset, 0, width() - mHistoryOffset, 100));
  if (mHistoryOffset)
    p.drawImage(QRect(width() - mHistoryOffset, -height(), mHistoryOffset, graphHeight), mHistoryImage,
