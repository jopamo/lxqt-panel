 ...
 * Configuration implementation for plugin-sysstat
 */
...
const QStringList LXQtSysStatConfiguration::msStatTypes = {
    QLatin1String(QT_TR_NOOP("CPU")), QLatin1String(QT_TR_NOOP("Memory")), QLatin1String(QT_TR_NOOP("Network"))};
...
LXQtSysStatConfiguration::LXQtSysStatConfiguration(PluginSettings* settings, QWidget* parent)
  ...
  connect(ui->typeCOB, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &LXQtSysStatConfiguration::saveSettings);
  connect(ui->intervalSB, static_cast<void (QDoubleSpinBox::*)(double)>(&QDoubleSpinBox::valueChanged), this,
          &LXQtSysStatConfiguration::saveSettings);
  connect(ui->sizeSB, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this,
          &LXQtSysStatConfiguration::saveSettings);
  connect(ui->linesSB, static_cast<void (QSpinBox::*)(int)>(&QSpinBox::valueChanged), this,
          &LXQtSysStatConfiguration::saveSettings);
  ...
  connect(ui->sourceCOB, static_cast<void (QComboBox::*)(int)>(&QComboBox::currentIndexChanged), this,
          &LXQtSysStatConfiguration::saveSettings);
...
void LXQtSysStatConfiguration::loadSettings() {
  ...
  ui->maximumHS->setValue(PluginSysStat::netSpeedFromString(
      settings().value(QStringLiteral("net/maximumSpeed"), QStringLiteral("1 MB/s")).toString()));
...
void LXQtSysStatConfiguration::saveSettings() {
  ...
  //  occurring in static finalization time (don't know the real reason...maybe ordering of static
  //  finalizers/destructors)
  QString type = QString::fromUtf8(
      ui->typeCOB->itemData(ui->typeCOB->currentIndex(), Qt::UserRole).toString().toStdString().c_str());
  settings().setValue(QStringLiteral("data/type"), type);
  settings().setValue(QStringLiteral("data/source"),
                      ui->sourceCOB->itemData(ui->sourceCOB->currentIndex(), Qt::UserRole));
...
void LXQtSysStatConfiguration::on_customColoursB_clicked() {
  ...
  colours[QStringLiteral("grid")] =
      QColor(settings().value(QStringLiteral("grid/colour"), defaultColours[QStringLiteral("grid")].name()).toString());
  colours[QStringLiteral("title")] = QColor(
      settings().value(QStringLiteral("title/colour"), defaultColours[QStringLiteral("title")].name()).toString());

  colours[QStringLiteral("cpuSystem")] =
      QColor(settings()
                 .value(QStringLiteral("cpu/systemColour"), defaultColours[QStringLiteral("cpuSystem")].name())
                 .toString());
  colours[QStringLiteral("cpuUser")] = QColor(
      settings().value(QStringLiteral("cpu/userColour"), defaultColours[QStringLiteral("cpuUser")].name()).toString());
  colours[QStringLiteral("cpuNice")] = QColor(
      settings().value(QStringLiteral("cpu/niceColour"), defaultColours[QStringLiteral("cpuNice")].name()).toString());
  colours[QStringLiteral("cpuOther")] =
      QColor(settings()
                 .value(QStringLiteral("cpu/otherColour"), defaultColours[QStringLiteral("cpuOther")].name())
                 .toString());
  colours[QStringLiteral("cpuFrequency")] =
      QColor(settings()
                 .value(QStringLiteral("cpu/frequencyColour"), defaultColours[QStringLiteral("cpuFrequency")].name())
                 .toString());

  colours[QStringLiteral("memApps")] = QColor(
      settings().value(QStringLiteral("mem/appsColour"), defaultColours[QStringLiteral("memApps")].name()).toString());
  colours[QStringLiteral("memBuffers")] =
      QColor(settings()
                 .value(QStringLiteral("mem/buffersColour"), defaultColours[QStringLiteral("memBuffers")].name())
                 .toString());
  colours[QStringLiteral("memCached")] =
      QColor(settings()
                 .value(QStringLiteral("mem/cachedColour"), defaultColours[QStringLiteral("memCached")].name())
                 .toString());
  colours[QStringLiteral("memSwap")] = QColor(
      settings().value(QStringLiteral("mem/swapColour"), defaultColours[QStringLiteral("memSwap")].name()).toString());

  colours[QStringLiteral("netReceived")] =
      QColor(settings()
                 .value(QStringLiteral("net/receivedColour"), defaultColours[QStringLiteral("netReceived")].name())
                 .toString());
  colours[QStringLiteral("netTransmitted")] = QColor(
      settings()
          .value(QStringLiteral("net/transmittedColour"), defaultColours[QStringLiteral("netTransmitted")].name())
          .toString());
