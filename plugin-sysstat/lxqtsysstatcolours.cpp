/* plugin-sysstat/lxqtsysstatcolours.cpp
 * Implementation file for lxqtsysstatcolours
 */
...
LXQtSysStatColours::LXQtSysStatColours(QWidget* parent)
    : QDialog(parent), ui(new Ui::LXQtSysStatColours), mSelectColourMapper(new QSignalMapper(this)) {
...
void LXQtSysStatColours::selectColour(const QString& name) {
  ...
  if (color.isValid()) {
    mColours[name] = color;
    mShowColourMap[name]->setStyleSheet(
        QStringLiteral("background-color: %1;\ncolor: %2;")
            .arg(color.name())
            .arg((color.toHsl().lightnessF() > 0.5) ? QStringLiteral("black") : QStringLiteral("white")));
...
void LXQtSysStatColours::applyColoursToButtons() {
  ...
  for (Colours::ConstIterator I = mColours.constBegin(); I != M; ++I) {
    ...
    mShowColourMap[I.key()]->setStyleSheet(
        QStringLiteral("background-color: %1;\ncolor: %2;")
            .arg(color.name())
            .arg((color.toHsl().lightnessF() > 0.5) ? QStringLiteral("black") : QStringLiteral("white")));
  }
