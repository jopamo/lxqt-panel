...
class LXQT_PANEL_API ILXQtPanelPlugin {
 public:
  ...
  ILXQtPanelPlugin(const ILXQtPanelPluginStartupInfo& startupInfo)
      : mSettings(startupInfo.settings), mPanel(startupInfo.lxqtPanel), mDesktopFile(startupInfo.desktopFile) {}
...
/**
Every plugin must have the ILXQtPanelPluginLibrary loader. You should only reimplement the instance() method which
