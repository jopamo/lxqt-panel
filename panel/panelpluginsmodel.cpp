...
void PanelPluginsModel::addPlugin(OneG4Panel* panel, const OneG4::PluginInfo& desktopFile) {
  if (dynamic_cast<OneG4PanelApplication const*>(qApp)->isPluginSingletonAndRunning(desktopFile.id()))
    return;
...
void PanelPluginsModel::loadPlugins(OneG4Panel* panel, QStringList const& desktopDirs) {
  ...
  for (auto const& name : std::as_const(plugin_names)) {
    ...
    if (QStringLiteral("screensaver") == type) {
      ...
      const QString& lock_desktop = QStringLiteral(ONEG4_LOCK_DESKTOP);
...
QPointer<Plugin> PanelPluginsModel::loadPlugin(OneG4Panel* panel,
                                               OneG4::PluginInfo const& desktopFile,
...
void PanelPluginsModel::onConfigurePlugin(QModelIndex const& index) {
  ...
  if (nullptr != plugin && (IOneG4PanelPlugin::HaveConfigDialog & plugin->iPlugin()->flags()))
