 ...
 * all Plugins should be created and handled by OneG4Panels.
 *
 * OneG4Panel is just the panel, it does not incorporate any functionality.
 * Each function of the panel is implemented by Plugins, even the mainmenu
 * (plugin-mainmenu) and the taskbar (plugin-taskbar). So the OneG4Panel is
 ...
 *
 * OneG4Panel itself is a window (QFrame/QWidget) and this class is mainly
 ...
 * is outsourced in PanelPluginsModel and OneG4PanelLayout. PanelPluginsModel
 * is responsible for loading/creating and handling the plugins.
 * OneG4PanelLayout is inherited from QLayout and set as layout to the
 * background of OneG4Panel, so OneG4PanelLayout is responsible for the
 * layout of all the Plugins.
 *
 * \sa OneG4PanelApplication, Plugin, PanelPluginsModel, OneG4PanelLayout.
 */
class ONEG4_PANEL_API OneG4Panel : public QFrame, public IOneG4Panel {
  Q_OBJECT
 ...
 public:
  ...
  /**
   * @brief Creates and initializes the OneG4Panel. Performs the following
   * steps:
   ...
   * in the config file.
   * @param settings The settings instance of this oneg4 panel application.
   * @param parent Parent QWidget, can be omitted.
   */
  OneG4Panel(const QString& configGroup, OneG4::Settings* settings, QWidget* parent = nullptr);
  virtual ~OneG4Panel();
   ...
   * shown together, this menu has to be created by OneG4Panel.
  ...
  // IOneG4Panel overrides ........
  IOneG4Panel::Position position() const override { return mPosition; }
  ...
  QString screenName() const { return QString(); }
  // ........ end of IOneG4Panel overrides

  /**
   * @brief Searches for a Plugin in the Plugins-list of this panel. Takes
   * an IOneG4PanelPlugin as parameter and returns the corresponding Plugin.
   * @param iPlugin IOneG4PanelPlugin that we are looking for.
   * @return The corresponding Plugin if it is loaded in this panel, nullptr
   * otherwise.
   */
  Plugin* findPlugin(const IOneG4PanelPlugin* iPlugin) const;
  ...
  /**
   * @brief Checks if this OneG4Panel can be placed at a given position
   ...
   */
  static bool canPlacedOn(int screenNum, OneG4Panel::Position position);
  /**
   ...
   * \sa strToPosition()
   */
  static QString positionToStr(IOneG4Panel::Position position);
  /**
   * @brief Returns an IOneG4Panel::Position from the given string. This can
   * be used to retrieve IOneG4Panel::Position values from the config files.
   * @param str string that should be converted to IOneG4Panel::Position
   * @param defaultValue value that will be returned if the string can not
   * be converted to an IOneG4Panel::Position.
   * @return IOneG4Panel::Position that was determined from str or
   * defaultValue if str could not be converted.
   *
   * \sa positionToStr()
   */
  static IOneG4Panel::Position strToPosition(const QString& str, IOneG4Panel::Position defaultValue);

  // Settings
  int iconSize() const override { return mIconSize; }    //!< Implement IOneG4Panel::iconSize().
  int lineCount() const override { return mLineCount; }  //!< Implement IOneG4Panel::lineCount().
  int panelSize() const { return mPanelSize; }
  int length() const { return mLength; }
  bool lengthInPercents() const { return mLengthInPercents; }
  OneG4Panel::Alignment alignment() const { return mAlignment; }
   ...
   * IOneG4PanelPlugin::SingleInstance flag set.
 ...
 public slots:
   ...
   * which handle the OneG4 hiding by resizing the panel.
   */
  void show();
  /**
   ...
   * because hiding the panel in OneG4 is done by making it very thin. So
   ...
   * is NOT the same as QWidget::hide() because hiding the panel in OneG4 is
 ...
 signals:
   ...
   * OneG4PanelApplication to get notified whenever an OneG4Panel should
   * be removed.
   * @param self This OneG4Panel. OneG4PanelApplication will use this
   * parameter to identify the OneG4Panel that should be removed.
   */
  void deletedByUser(OneG4Panel* self);
  /**
   ...
   * Currently, this signal is used by OneG4PanelApplication which
 ...
 protected:
   ...
   * method is called when a widget (in this case: the OneG4Panel) is
   * shown. The call could happen before and after the widget is shown.
   * This method is just overridden to get notified when the OneG4Panel
   * will be shown. Then, OneG4Panel will call realign().
 ...
 public slots:
   ...
   *
   * Furthermore, this method will be called by OneG4PanelApplication
   * when a new plugin gets added (the OneG4Panel instances are handled
   * by OneG4PanelApplication). That is why this method/slot has to be
   * public.
   */
 ...
 private slots:
   ...
   * 2. OneG4::Application::themeChanged(), i.e. when the user changes
   * the theme.
   */
  void realign();
  /**
   ...
   * OneG4PanelLayout::pluginMoved() will be connected to this slot so
 ...
 private:
  /**
   * @brief The OneG4PanelLayout of this panel. All the Plugins will be added
   * to the UI via this layout.
   */
  OneG4PanelLayout* mLayout;
  /**
   * @brief The OneG4::Settings instance as retrieved from
   ...
   * set. This background widget will have the OneG4PanelLayout mLayout which
   * will in turn contain all the Plugins.
   */
  QFrame* OneG4PanelWidget;
  /**
   ...
   */
  int findAvailableScreen(OneG4Panel::Position position);
  /**
   ...
   *
   * \sa IOneG4Panel::iconSize(), setIconSize().
   */
  int mIconSize;
  /**
   * @brief Stores the number of lines/rows of the panel.
   *
   * \sa IOneG4Panel::lineCount(), setLineCount().
   */
  int mLineCount;
   ...
   * shown on the configured screen, OneG4Panel will determine another
  ...
  OneG4::Settings* settings() const { return mSettings; }
};
