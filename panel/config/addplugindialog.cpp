...
AddPluginDialog::AddPluginDialog(QWidget* parent) : QDialog(parent), ui(new Ui::AddPluginDialog) {
  ...
  desktopFilesDirs
      << QString::fromLocal8Bit(qgetenv("LXQT_PANEL_PLUGINS_DIR")).split(QLatin1Char(':'), Qt::SkipEmptyParts);
  ...
  connect(dynamic_cast<LXQtPanelApplication*>(qApp), &LXQtPanelApplication::pluginAdded, this,
          &AddPluginDialog::filter);
  connect(dynamic_cast<LXQtPanelApplication*>(qApp), &LXQtPanelApplication::pluginRemoved, this,
          &AddPluginDialog::filter);
}
